...
// Crude but effective HTML → plain-text conversion for the multipart
// alternative. Spam filters penalise HTML-only mail heavily; providing
// a text part that mirrors the HTML content is one of the cheapest
// deliverability wins available.
...
async function sendOne({ to, subject, html, text, replyTo }) {
  ...
  // Always include a plain-text part. HTML-only emails are one of the
  // strongest spam-folder signals; auto-derive from the HTML when the
  // caller didn't supply one.
  const textOut = text || (html ? htmlToText(html) : '');
  if (textOut) body.set('text', textOut);

  // Reply-To: the strategist's real mailbox. The From stays the
  // authenticated Mailgun identity, but replies route to a human —
  // and a valid Reply-To on a real domain improves filter scoring.
  const replyToOut = replyTo || process.env.MAILGUN_REPLY_TO || '';
  if (replyToOut) body.set('h:Reply-To', replyToOut);

  // List-Unsubscribe headers — required by Gmail/Yahoo sender rules
  // (2024+) for anything they classify as bulk. Missing them is a
  // direct spam-folder routing signal. mailto: variant keeps this
  // dependency-free; requests land in the support inbox.
  const unsubMail = process.env.UNSUBSCRIBE_MAILTO || 'support@goconstellation.com';
  body.set('h:List-Unsubscribe', `<mailto:${unsubMail}?subject=unsubscribe>`);
...
function buildMergeRow({ recipient = {}, batch = null, sender = null, crmClient = null, overrides = {} } = {}) {
  ...
  const row = {
    ...
    //
    // Precedence (updated 2026-09-09 — "Client Actual Name" wiring):
    //   1. crmClient.client_actual_name — the OS CRM's dedicated field
    //      (top of Account IDs), read LIVE from public.client at send
    //      time. The deliberately-entered human name always wins.
    //   2. recipient.first_name — synced/mirrored copy (ClickUp custom
    //      field or the CRM dual-write into the Sender table).
    //   3. recipient.firm       — the firm custom field, when populated.
    //   4. recipient.name       — the ClickUp task title (always set).
    first_name:         (crmClient?.client_actual_name || '').trim()
                          || (recipient.first_name || '').trim()
                          || recipient.firm || recipient.name || '',

    // {{client_actual_name}} — also exposed as its own merge tag for
    // templates that want the person's name somewhere other than the
    // greeting. Empty string when not set in the OS CRM.
    client_actual_name: (crmClient?.client_actual_name || '').trim(),
    ...
    // {{report_month}} / {{report_month_year}} — the month a monthly
    // report is ABOUT. Reports go out the first week of the following
    // month, so this is always the previous calendar month. Alias of
    // last_month, but reads correctly in templates: "your {{report_month}}
    // report" can never silently mean the month the email was sent in.
    // {{leads}} is fetched for this same month (routes/api.js).
    report_month:       monthNames[lastMonthDate.getMonth()],
    report_month_year:  `${monthNames[lastMonthDate.getMonth()]} ${lastMonthDate.getFullYear()}`,
