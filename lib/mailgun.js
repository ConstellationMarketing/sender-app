...
function buildMergeRow({ recipient = {}, batch = null, sender = null, crmClient = null, overrides = {} } = {}) {
  ...
  const row = {
    // ── Client identity ─────────────────────────────────────
    name:               recipient.name || '',
    // {{first_name}} — prefer the explicit contact-first-name field
    // populated from ClickUp during /clients-sync. Falls back to the
    // firstWord(name) heuristic (which grabbed the firm's first word)
    // only when the ClickUp field is empty, matching the pre-existing
    // behavior so no batch regresses.
    first_name:         (recipient.first_name || '').trim() || firstWord(recipient.name),
