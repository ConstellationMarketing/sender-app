...
function buildMergeRow({ recipient = {}, batch = null, sender = null, crmClient = null, overrides = {} } = {}) {
  ...
  const row = {
    ...
    //
    // Precedence (updated 2026-09-09, Omar): greetings use the FIRM name.
    // The short-lived Client Actual Name preference (same day) is
    // reverted — the team decided emails should address the firm, not a
    // person, matching Camila's 2026-08-20 direction:
    //   1. recipient.firm — the firm custom field, when populated.
    //   2. recipient.name — the ClickUp task title (always set).
    // The person's name stays available to templates via the dedicated
    // {{client_actual_name}} tag below, but never drives {{first_name}}.
    first_name:         (recipient.firm || '').trim() || recipient.name || '',
