...
// Firm-name lookup — the client-firm the email is sent to. Uses EXACT
// case-insensitive equality (not substring) because a substring match on
// 'firm' picked up unrelated fields like "Firm Language" whose value is
// "English" / "Spanish", and every recipient ended up with firm='English'.
// Ordered by preference; first non-empty exact match wins.
const FIRM_FIELD_EXACT_NAMES = [
  'firm name',
  'law firm',
  'firm',
  'company name',
...
const FIRST_NAME_FIELD_NEEDLES = [
  // "Client Actual Name" — the OS CRM's dedicated field (top of Account
  // IDs, 2026-09-09), mirrored to a ClickUp custom field of the same
  // name. Highest priority: it's the deliberately-entered human name.
  'client actual name',
  'client first name',
...
// Same as findCustomField but with EXACT (case-insensitive) equality
// on the field name. Used for the firm lookup so unrelated fields like
// "Firm Language" don't bleed into the firm column.
function findCustomFieldExact(task, exactNames) {
  if (!Array.isArray(task.custom_fields)) return '';
  const wanted = exactNames.map(n => n.toLowerCase());
  // Walk needles in order — first match wins so preferred names take
  // precedence over generic ones ("Firm Name" beats "Firm").
  for (const want of wanted) {
    for (const f of task.custom_fields) {
      if (String(f.name || '').trim().toLowerCase() !== want) continue;
      if (f.value == null) continue;
      const v = typeof f.value === 'object' ? (f.value.email || f.value.text || '') : String(f.value);
      const s = v.trim();
      if (s) return s;
...
async function fetchActiveClients() {
  ...
  for (let page = 0; page < 20; page++) {
    ...
    for (const t of tasks) {
      ...
      const firm = findCustomFieldExact(t, FIRM_FIELD_EXACT_NAMES) || String(t.name || '');
