...
// Name of the newsletter-only "everyone we send to" list. Kept in a
// constant so index.html and this file agree.
const ALL_CLIENTS_LIST_NAME = 'All Clients (Live + Onboarding + Hosting)';
...
async function runAssigneeSync() {
  ...
  // Query the strategist lists AND the All Clients list in the same
  // round-trip. All Clients is the newsletter-only "everyone we send to"
  // list — populated below from recipients whose status is in
  // NEWSLETTER_STATUSES (includes hosting variants).
  const allSyncedListNames = [...STRATEGISTS, ALL_CLIENTS_LIST_NAME];
  const [clients, listsRes, recipientsRes, membersRes] = await Promise.all([
    fetchActiveClients(),
    sb.from('sender_clients_lists').select('id, name, newsletter_only').in('name', allSyncedListNames),
  ...
  // Strategist lists are is_fixed=true, newsletter_only=false.
  // All Clients is newsletter_only=true so the Monthly Reports modal
  // can filter it out of the audience dropdown.
  for (const name of STRATEGISTS) {
    if (!listByName.has(name)) {
      const ins = await sb
        .from('sender_clients_lists')
        .insert({ name, is_fixed: true, newsletter_only: false })
        .select('id, name')
        .single();
  ...
  if (!listByName.has(ALL_CLIENTS_LIST_NAME)) {
    const ins = await sb
      .from('sender_clients_lists')
      .insert({ name: ALL_CLIENTS_LIST_NAME, is_fixed: true, newsletter_only: true })
      .select('id, name')
      .single();
    if (ins.error) throw new Error(`create list ${ALL_CLIENTS_LIST_NAME}: ${ins.error.message}`);
    listByName.set(ins.data.name, ins.data.id);
  }

  const strategistListIds = new Set(STRATEGISTS.map(n => listByName.get(n)));
  const allClientsListId  = listByName.get(ALL_CLIENTS_LIST_NAME);
  const allSyncedListIds  = new Set([...strategistListIds, allClientsListId]);
  ...
  // ── Pass 2: All Clients list (live + onboarding + hosting) ──────
  //
  // Iterates EVERY recipient (not just CRM-matched ones), because the
  // All Clients list is anchored on recipient status alone — a hosting
  // client may not currently appear in fetchActiveClients() but should
  // still land in the newsletter list.
  for (const rec of recipientsById.values()) {
    const current = currentByRecipient.get(rec.id) || new Set();
    const inList = current.has(allClientsListId);
    const eligible = NEWSLETTER_STATUSES.has(rec.status);
    if (eligible && !inList) {
      toAdd.push({ recipient_id: rec.id, list_id: allClientsListId });
      counters.all_clients_added++;
    } else if (!eligible && inList) {
      toRemove.push({ recipient_id: rec.id, list_id: allClientsListId });
